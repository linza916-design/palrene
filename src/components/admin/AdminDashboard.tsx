...
export default function AdminDashboard() {
  ...
  const [activeSegment, setActiveSegment] = useState<"analytics" | "verifications" | "moderation" | "payments" | "ads">("analytics");
  ...
  const pendingVerifications = profiles.filter((p) => p.is_verified === false && p.verification_video_url && p.id !== "poly-ai");
      ...
      <div className="space-y-4 lg:col-span-1">
        <div className="p-4 rounded-3xl bg-neutral-50 dark:bg-zinc-950/45 border border-neutral-150/45 dark:border-neutral-900 shadow-sm space-y-4">
          <div className="space-y-1">
            ...
            <p className="text-[11px] text-neutral-400 font-mono">Operations workspace for auto-admins.</p>
          </div>

          <div className="flex flex-col gap-1.5 pt-2">
            {[
              { id: "analytics", label: "Operations Analytics", icon: LineChart },
              { id: "verifications", label: `Verify ID Reviews (${pendingVerifications.length})`, icon: Award },
              { id: "moderation", label: `Posts Moderation Logs (${reportedPostsList.length})`, icon: ShieldAlert },
              { id: "payments", label: `Revenue & Ledger (${payments.filter(p => p.status === 'successful').length})`, icon: CreditCard },
              { id: "ads", label: `Review Sponsored Ads (${pendingAds.length})`, icon: Megaphone }
            ].map((seg) => {
              const Icon = seg.icon;
      ...
      <div className="lg:col-span-3 space-y-6">
        ...
        {activeSegment === "analytics" && (
          <div className="space-y-6">
            <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-neutral-400">Palrene global workspace telemetry</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
              <div className="p-5 border border-neutral-200/50 dark:border-neutral-900 rounded-3xl bg-neutral-50 dark:bg-zinc-950/45">
                <span className="text-[10px] font-mono text-neutral-400 uppercase">Resonating Traffic / hr</span>
                <div className="flex items-end space-x-2.5 mt-2">
                  <span className="text-2xl font-bold dark:text-white">41.8K</span>
              ...
              <div className="p-5 border border-neutral-200/50 dark:border-neutral-900 rounded-3xl bg-neutral-50 dark:bg-zinc-950/45">
                <span className="text-[10px] font-mono text-neutral-450 uppercase">Subscribed Seekers</span>
                <div className="flex items-end space-x-2.5 mt-2">
                  <span className="text-2xl font-bold dark:text-white">5.1K</span>
                  <span className="text-[9px] font-mono text-green-500 flex items-center gap-0.5 mb-1 bg-green-500/10 px-1 rounded font-bold">
                    <TrendingUp size={9} /> +4%
                  </span>
                </div>
              ...
              <div className="p-5 border border-neutral-200/50 dark:border-neutral-900 rounded-3xl bg-neutral-50 dark:bg-zinc-950/45">
                <span className="text-[10px] font-mono text-neutral-400 uppercase">Poly advice requests</span>
                <div className="flex items-end space-x-2.5 mt-2">
                  <span className="text-2xl font-bold dark:text-white">12.9K</span>
              ...
              <div className="p-5 border border-neutral-200/50 dark:border-neutral-900 rounded-3xl bg-neutral-50 dark:bg-zinc-950/45 relative overflow-hidden group hover:border-red-500/30 transition">
                <span className="text-[10px] font-mono text-neutral-400 uppercase">Reported Ripples</span>
                <div className="flex items-end space-x-2.5 mt-2">
                  <span className="text-2xl font-bold text-red-500">{reportedPostsList.length}</span>
            ...
            <div className="p-6 border border-neutral-150/45 dark:border-neutral-900 rounded-3xl bg-neutral-50 dark:bg-zinc-950/30 space-y-4">
              <span className="text-[10px] font-mono text-neutral-450 uppercase block">Weekly Active Match Waves</span>
              <div className="h-44 flex items-end justify-between items-stretch gap-2.5 pt-4">
                {[45, 62, 75, 48, 85, 91, 100].map((val, idx) => (
                  <div key={idx} className="flex-1 flex flex-col justify-end items-center">
                    <div 
                      className="w-full bg-gradient-to-t from-red-500 to-orange-500 rounded-lg transition-all duration-[1000ms]"
                      style={{ height: `${val}%` }}
                    />
                    <span className="text-[8px] font-mono text-neutral-400 mt-2">Day {idx + 1}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {activeSegment === "verifications" && (
          <div className="space-y-4">
            <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-neutral-400">Identity verification matching review queues</h3>
                ...
                {pendingVerifications.map((p) => (
                  ...
                  >
                    <div className="flex items-center space-x-3.5">
                      ...
                      <div>
                        <h4 className="font-bold text-neutral-800 dark:text-white">{p.full_name}</h4>
                        <p className="text-[9px] font-mono text-neutral-400">@{p.username}</p>
                      </div>
                    </div>
                    ...
                    <div className="p-3 bg-neutral-100 dark:bg-zinc-900 rounded-xl space-y-3 mt-2 text-left border border-neutral-150/20 dark:border-zinc-850">
                      <p className="text-[10px] uppercase tracking-wider font-mono font-bold text-orange-500">Identity Files review</p>
                      
                      <div className="space-y-3">
                        {/* 30-Second Verification video segment */}
                        <div>
                          <span className="text-[9px] font-mono text-neutral-400 dark:text-neutral-500 block mb-1">30s Selfie Clip Verification:</span>
                          {p.verification_video_url ? (
                            ...
                            <div className="rounded-xl overflow-hidden p-2 bg-zinc-900 border border-zinc-850 text-center space-y-1.5">
                              <p className="text-[9.5px] italic text-neutral-400 font-serif">[Simulated 30s verification video loaded in live sandbox]</p>
                              <video 
                        ...
                        <div className="grid grid-cols-2 gap-2 pt-1">
                          <div>
                            <span className="text-[9px] font-mono text-neutral-400 dark:text-neutral-500 block mb-1">ID Document Front:</span>
                            <img 
                              src={p.verification_doc_front_url || "https://images.unsplash.com/photo-1554774853-aae0a22c8aa4?w=450&auto=format&fit=crop&q=80"}
                            ...
                            {!p.verification_doc_front_url && (
                              <span className="text-[8px] font-mono text-neutral-450 italic mt-0.5 block text-center">[Mock front ID review]</span>
                            )}
                          </div>
                          <div>
                            <span className="text-[9px] font-mono text-neutral-400 dark:text-neutral-500 block mb-1">ID Document Back:</span>
                            <img 
                              src={p.verification_doc_back_url || "https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?w=450&auto=format&fit=crop&q=80"}
                              alt="Passport ID Back preview"
                              className="rounded-xl border border-neutral-250 dark:border-zinc-800 w-full object-cover aspect-video bg-neutral-950"
                              referrerPolicy="no-referrer"
                            />
                            {!p.verification_doc_back_url && (
                              <span className="text-[8px] font-mono text-neutral-455 text-neutral-450 italic mt-0.5 block text-center">[Mock back ID review]</span>
                            )}
                          </div>
                        </div>
        ...
        {activeSegment === "moderation" && (
          <div className="space-y-4">
            <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-neutral-400">Reported ripples moderation log queue</h3>
              ...
              {reportedPostsList.length === 0 ? (
                <div className="p-8 border border-dashed border-neutral-200 dark:border-neutral-800 rounded-3xl bg-neutral-50 dark:bg-zinc-950/20 text-center py-12">
                  <p className="text-xs text-neutral-400 font-mono">
                    Moderation queue clean. All connections operate within compliance.
                  </p>
                </div>
              ) : (
                reportedPostsList.map((post) => (
                  ...
                  >
                    <div className="space-y-2">
                      ...
                      <span className="text-[9px] font-mono text-neutral-450 block">By @{post.profile.username}</span>
                    </div>
        ...
        {activeSegment === "payments" && (
          <div className="space-y-6 animate-fade-in text-left">
            <div>
              <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-neutral-400">Palrene Financial telemetry and secure subscriber ledger</h3>
              <p className="text-[11px] text-neutral-500 mt-0.5">Audit transaction flows, process Flutterwave ledger refunds, and enforce safety bans.</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="p-4 rounded-2xl border border-neutral-150/50 dark:border-neutral-900 bg-neutral-50 dark:bg-zinc-950/20 text-xs">
                <span className="text-[10px] uppercase font-mono text-neutral-400 font-bold block">Gross Ledger Sales</span>
                <span className="text-xl font-bold font-mono text-green-500 block mt-1">
                  ${payments.filter(p => p.status === "successful").reduce((acc, curr) => acc + curr.amount, 0).toFixed(2)} USD
                </span>
                <p className="text-[9px] text-neutral-450 mt-0.5">Settled via Flutterwave integrations.</p>
              </div>

              <div className="p-4 rounded-2xl border border-neutral-150/50 dark:border-neutral-900 bg-neutral-50 dark:bg-zinc-950/20 text-xs">
                <span className="text-[10px] uppercase font-mono text-neutral-400 font-bold block">Total Funds Refunded</span>
                <span className="text-xl font-bold font-mono text-red-500 block mt-1">
                  ${payments.filter(p => p.status === "refunded").reduce((acc, curr) => acc + curr.amount, 0).toFixed(2)} USD
                </span>
                <p className="text-[9px] text-neutral-455 mt-0.5 text-neutral-450">Subscription rights revoked.</p>
              </div>

              <div className="p-4 rounded-2xl border border-neutral-150/50 dark:border-neutral-900 bg-neutral-50 dark:bg-zinc-950/20 text-xs">
                <span className="text-[10px] uppercase font-mono text-neutral-400 font-bold block">Active Paid Circles</span>
                <span className="text-xl font-bold font-mono text-orange-500 block mt-1">
                  {profiles.filter(p => p.subscription_tier && p.subscription_tier !== "Free").length} orbits
                </span>
                <p className="text-[9px] text-neutral-450 mt-0.5">Starter & Pro tiers active.</p>
              </div>
            </div>
                    ...
                    >
                      <div className="space-y-1.5 text-left">
                        <div className="flex items-center gap-2">
                          <span className="font-extrabold text-neutral-850 dark:text-neutral-100 font-serif">{tx.userName}</span>
                          <span className={`px-2 py-0.2 text-[8px] font-bold tracking-wider rounded font-mono ${
                            tx.plan === "Pro" 
                        ...
                        <div className="flex gap-1.5">
                          {tx.status === "successful" ? (
                            <>
                              <button
                                onClick={() => {
                                  if (confirm(`Revoke premium subscription and trigger Flutterwave refund of $${tx.amount.toFixed(2)} to ${tx.userName}?`)) {
                                    refundPaymentTransaction(tx.id);
                                    alert("Settlement canceled. Subscription downgraded and refund triggered.");
                                  }
                                }}
                              ...
                              <button
                                onClick={() => {
                                  if (confirm(`Do you wish to completely disable billing parameters & blacklist ${tx.userName}?`)) {
                                    disableSubscription(tx.userId);
                                    alert("User down-graded and custom constraints active.");
                                  }
                                }}
        ...
        {activeSegment === "ads" && (
          <div className="space-y-4 text-left">
            <div>
              <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-neutral-400">Review Sponsored Advertising Spots</h3>
              <p className="text-[11px] text-neutral-500 mt-0.5">Audit ad graphics and pitches submitted by Pro Seekers before placing them live.</p>
            </div>
                  ...
                  return (
                    <div 
                      ...
                      className={`p-4 border rounded-3xl bg-neutral-50 dark:bg-zinc-950/30 flex flex-col sm:flex-row gap-4 text-xs relative overflow-hidden transition-all duration-300 ${
                        ad.status === "pending" ? "border-orange-500/30 ring-1 ring-orange-500/10" : "border-neutral-150/45 dark:border-neutral-900"
                      }`}
                    >
                      ...
                      <div className="flex-1 space-y-1.5">
                        <div className="flex justify-between items-center">
                          <span className={`text-[8.5px] font-mono font-extrabold uppercase px-1.5 py-0.5 rounded border ${
                            ...
                            {ad.status === "approved" ? "● Approved & Live" : ad.status === "rejected" ? "● Disallowed" : "● PENDING AUDIT"}
                          </span>

                          <span className="text-[9.5px] font-mono text-neutral-400">Owner: @{creator?.username || "Companion Soul"}</span>
                        </div>

                        <div>
                          <h4 className="font-extrabold text-neutral-850 dark:text-neutral-100 font-serif text-sm leading-tight">{ad.title}</h4>
                          <p className="text-[11px] text-neutral-550 leading-relaxed italic mt-0.5 font-serif">"{ad.description}"</p>
                        </div>
                          ...
                          {ad.status === "pending" && (
                            <div className="flex gap-1.5">
                              <button
                                onClick={() => {
                                  approveAd(ad.id);
                                  alert("Ad approved! Live placement established across core Category headers.");
                                }}
                              ...
                              <button
                                onClick={() => {
                                  rejectAd(ad.id);
                                  alert("Ad promotion disallowed and dismissed.");
                                }}
