...
export default function UnsplashModal({ isOpen, onClose, onSelectMultiple }: UnsplashModalProps) {
  const [search, setSearch] = useState("");
  const [images, setImages] = useState<any[]>([]);
  const [selectedUrls, setSelectedUrls] = useState<string[]>([]);
  const [loading, setLoading] = useState(false);
  
  const ACCESS_KEY = (import.meta as any).env?.VITE_UNSPLASH_ACCESS_KEY || "q4SkW5ZwIn0qb-T716jeRqLaiS33nKSO0aZteFgOzhw";
  ...
  const fetchEditorial = async () => {
    setLoading(true);
    try {
      const res = await fetch(`https://api.unsplash.com/photos?client_id=${ACCESS_KEY}&per_page=12&order_by=popular`);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`https://api.unsplash.com/search/photos?client_id=${ACCESS_KEY}&query=${encodeURIComponent(search)}&per_page=12`);
      const data = await res.json();
          ...
          >
            ...
            <div className="flex items-center justify-between p-5 border-b border-neutral-800 bg-neutral-950/40">
              <div>
                ...
                <p className="text-xs text-neutral-400 mt-0.5">Select multiple images to attach to your post.</p>
              </div>
              <button
                onClick={onClose}
            ...
            {/* Search Input */}
            <form onSubmit={handleSearch} className="p-4 border-b border-neutral-800 bg-neutral-900/60 flex gap-2">
              ...
              <button
                type="submit"
                className="px-5 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-red-500 to-orange-500 rounded-xl hover:from-red-600 hover:to-orange-600 shadow"
              >
                Seek
              </button>
            </form>
            ...
            <div className="p-5 overflow-y-auto max-h-[380px]">
              {loading ? (
                <div className="flex flex-col items-center justify-center py-20 space-y-3">
                  ...
                  <p className="text-xs text-neutral-400 font-mono">Syncing lens...</p>
                </div>
              ) : images.length === 0 ? (
                    ...
                    return (
                      <div
                        ...
                        className={`relative overflow-hidden cursor-pointer rounded-xl aspect-[4/3] bg-neutral-950 border transition-all duration-300 group ${
                          isSelected ? "ring-2 ring-orange-500 border-transparent scale-98" : "border-neutral-800/40 hover:border-neutral-700 hover:scale-101"
                        }`}
                      >
                        <img
                          src={img.urls.thumb}
                        ...
                        />
                        {/* Selector indicator */}
                        <div className={`absolute top-2.5 right-2.5 w-6 h-6 rounded-full flex items-center justify-center border text-white transition-all ${
                          isSelected ? "bg-orange-500 border-transparent" : "bg-black/40 border-white/20 opacity-0 group-hover:opacity-100"
                        }`}>
                        ...
                        <div className="absolute inset-x-0 bottom-0 p-2 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                          <p className="text-[10px] text-white/80 font-mono truncate">By {img.user.name}</p>
                        </div>
                      </div>
                    );
                  })}
            ...
            <div className="flex items-center justify-between p-4 border-t border-neutral-800 bg-neutral-950/40">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-5 py-2 text-xs font-mono font-bold text-white bg-gradient-to-r from-red-500 to-orange-500 rounded-xl hover:from-red-600 hover:to-orange-600 transition disabled:opacity-45 disabled:pointer-events-none"
                >
                  Attach to Post
                </button>
              </div>
