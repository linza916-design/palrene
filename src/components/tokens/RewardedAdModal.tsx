          ...
          >
            <AnimatePresence mode="wait">
              {phase === "watching" ? (
                ...
                >
                  {/* Ad display */}
                  <div className={`relative h-56 bg-gradient-to-br ${ad.color} flex flex-col items-center justify-center p-6`}>
                    ...
                    <div className="absolute top-3 right-3 flex items-center gap-1.5 bg-black/30 backdrop-blur-sm rounded-full px-3 py-1">
                      <span className="text-amber-400 text-xs font-bold">+15 tokens</span>
                    </div>
                    ...
                    <h3 className="text-white font-bold text-xl text-center mb-1">{ad.headline}</h3>
                    <p className="text-white/60 text-sm text-center">{ad.subline}</p>
                    <p className="text-white/40 text-xs mt-3">{ad.brand}</p>
                  </div>

                  {/* Progress bar and controls */}
                  <div className="bg-zinc-900 p-5">
                    <div className="relative h-1.5 bg-white/10 rounded-full mb-4 overflow-hidden" role="progressbar" aria-valuenow={100 - countdown * (100/15)} aria-valuemin={0} aria-valuemax={100}>
                      <motion.div
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        aria-label={canSkip ? "Skip advertisement" : `Skip available in ${countdown - 10}s`}
                      >
                        {canSkip ? "Skip" : `Skip in ${Math.max(0, countdown - 10)}s`}
                      </button>
                    </div>
                  </div>
                </motion.div>
                ...
                >
                  ...
                  >
                    <h3 className="text-2xl font-black text-white mb-1">+15 Tokens!</h3>
                    <p className="text-white/50 text-sm mb-6">You've earned your reward. Keep watching ads to earn more!</p>
